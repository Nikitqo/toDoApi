...
async def create_board(board, user_id):  # пока оставил user_id, может вместо юзера надо будет добавить роли?
...
async def delete_board_by_id(id, user_id):
    try:
        task = await find_board_by_id(id)
